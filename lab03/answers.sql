...
users.username, users.first_name, users.last_name FROM posts 
INNERJOIN users ON posts.user_id = users.id 
WHERE users.id = 12 or users.id =26;



-- Exercise 9
SELECT p.id, p.pub_date, f.following_id FROM following f 
...
-- Exercise 12
DELETE FROM bookmarks WHERE user_id = 26 
and post_id IN (219, 220, 221);
